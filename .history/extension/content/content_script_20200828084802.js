    ...
    addText() {
        if (this.textOnButton) {
            if (this.textOnButton.includes('&#')) {
                this.span.innerHTML = this.textOnButton;
                return true;
            } else {
                this.span.innerText = this.textOnButton.toUpperCase();
                return true;
            }
...
const createLoadingCircle = (body) => {
    ...
    circle.setAttribute('src', chrome.runtime.getURL('content/img/logo.png'));
